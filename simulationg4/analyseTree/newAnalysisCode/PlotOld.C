...
int main()
{
  //For data where y and z are flipped
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++)
      ...
      for(int i=0;i<myReader.InitElecRadius->size();i++)
	{
	  ...
	  OldVector.SetMagThetaPhi(myReader.InitElecRadius->at(i),std::acos(myReader.InitElecCosTheta->at(i)),myReader.InitElecPhi->at(i));
	  TVector3 NewVector = TVector3(OldVector.x(),OldVector.z(),OldVector.y());
	  //std::cout << NewVector.CosTheta();
	  CosTheta.push_back(NewVector.CosTheta());
	  Radius.push_back(myReader.InitElecRadius->at(i));
	  SinTheta.push_back(sqrt(1 - pow(CosTheta[i],2)));
	  h_nElecRadius->Fill(Radius[i]);
	  h_nElecPhi->Fill(NewVector.Phi());
      ...
      for (int i=nPulse.size()-1; i>0; i--)
	{
      ...
      // Elec Position and Pulse Histograms
      ...
      for(int i=0;i<myReader.InitIonRadius->size();i++)
	{
      ...
      //double PulseIntegral = h_nPulse1Event->Integral("width");
