...
class ViewDrift : public ViewBase {
 public:
  ...
  int GetNExcitationMarker(){return m_exc.size();};
  int GetNIonisationMarker(){return m_ion.size();};
  void GetExcitationMarker(const int i, double& x, double& y, double& z){x=m_exc[i].at(0); y=m_exc[i].at(1); z=m_exc[i].at(2);};
  void GetIonisationMarker(const int i, double& x, double& y, double& z){x=m_ion[i].at(0); y=m_ion[i].at(1); z=m_ion[i].at(2);};
