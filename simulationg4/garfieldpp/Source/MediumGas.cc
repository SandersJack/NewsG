...
bool MediumGas::SetComposition(const std::vector<std::string> gas, const std::vector<double> f) {

  std::vector<std::string> gases(gas);
  std::vector<double> fractions(f);

  std::array<std::string, 6> gasesIn;
  std::array<double, 6> fractionsIn;

  if(fractions.size()<gases.size()-1)
    return false;
  if(fractions.size()>gases.size())
    return false;
  if(fractions.size()==gases.size()-1)
  ...
  if(imax>6)
    {
      imax=6;
      std::cout << m_className << "::SetComposition:\n    " << "Will only read first 6 entries"<<std::endl;
    }
  for(unsigned int i=0;i<gases.size();i++)
    {
      gasesIn[i]=gases[i];
      fractionsIn[i]=fractions[i];
    }

  return SetComposition(gasesIn,fractionsIn);
}

bool MediumGas::SetComposition(const std::array<std::string, 6> gases, const std::array<double, 6> fractions) {
...
bool MediumGas::SetComposition(const std::string& gas1, const double f1,
  ...
  return SetComposition(gases,fractions);
}
...
bool MediumGas::MergeGasFile(const std::string& filename,
  ...
  if (iemode * ibmode * iamode == 0) {
    ...
    if (existing[14] && gasok[14] && !excMatch) {
      ...
      //KOSTAS if the don't match keep the old ones
      //m_excLevels.clear();
      //m_excRates.clear();
      //existing.reset(14);
      gasok.reset(14);
