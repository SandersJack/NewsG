...
UserG4FastSimulationModel::UserG4FastSimulationModel(const G4String &name, G4Region* region, InitialisationParameters* params) :
  G4VFastSimulationModel(name, region),
  m_sensor(new Garfield::Sensor()),
  m_initialElectrons(new Ionisation(new TRandom3(0))),
  m_avalanche(new ElectronAvalanche()),
  m_drift(new IonDrift()),
  m_pulseProcess(new Electronics()),
  signalNt(m_signalnBins,0),
  signaltNt(m_signalnBins,0)
{
  //std::cout << "[UserG4FastSimulationModel] Creating User Model!" << std::endl;
...
{ 
  ...
  delete m_avalanche;
  delete m_drift;
  delete m_pulseProcess;
}



G4bool UserG4FastSimulationModel::IsApplicable(const G4ParticleDefinition& particleType)
{
  //Case B
  // std::cout << "[UserG4FastSimulationModel] IsApplicable() has been used" << std::endl;
...
{
  //Case B
  //std::cout << "[UserG4FastSimulationModel] ModelTrigger() has been used" << std::endl;
...
void UserG4FastSimulationModel::DoIt(const G4FastTrack& fastTrack, G4FastStep& fastStep)
{  
  ...
  TStopwatch timer;
  timer.Start();
  //std::cout << "[UserG4FastSimulation] Starting Garfield++ simulation" << std::endl;
  ...
  //std::cout << "[UserG4FastSimulationModel] SD collection name is: " << sensDet->GetCollectionName(0) << std::endl;

  const G4ThreeVector localPosition = fastTrack.GetPrimaryTrackLocalPosition(); // Same as global position
  const G4ThreeVector globalPosition = fastTrack.GetPrimaryTrack()->GetPosition(); 
  G4ThreeVector localDirection = fastTrack.GetPrimaryTrackLocalDirection();
  double globalTime = fastTrack.GetPrimaryTrack()->GetGlobalTime(); //Same as local time
  ...
  //std::cout << "start of garfield " << x/cm << " " << y/cm << " " << z/cm << std::endl;
 ...
 // Remove any electrons not in the gas volume
  ...
  if(!m_sensor->GetMedium(x/cm, y/cm, z/cm, gasDummy))
    {
      std::cout << "[UserG4FSM] No medium found at initial position! Skipping Garfield." << std::endl;
      return;
    }
  else if (!gasDummy->IsIonisable())
    {
      std::cout << "[UserG4FSM] Medium exists but is non-ionisable! Skipping Garfield." << std::endl;
      return;
    }
  ...
  //   {
  //     std::cout << "[UserG4FSM] Electron is not in sphere - skipping Garfield" << std::endl;
  //     return;
  //   }
  // else if(y/cm < -0.1501 && sqrt(x/cm*x/cm + z/cm*z/cm) < 0.3001)
  //   {
  //     std::cout << "[UserG4FSM] Electron is in bar - skipping Garfield" << std::endl;
  //     return;
  //   }
  
  Garfield::TrackHeed* trackHeed = new Garfield::TrackHeed();
  trackHeed->EnableDebugging();
  trackHeed->EnableDeltaElectronTransport();
  trackHeed->SetSensor(m_sensor);
  ...
  //Garfield++ requires the variables be in certain units, hence the conversion factors (e.g /cm)
  trackHeed->TransportDeltaElectron(x/cm, y/cm, z/cm, globalTime/ns, eKinetic/eV, dx, dy, dz, ne);
  //std::cout << "[UserG4FastSimulationModel] Number of HEED generated electrons is: " << ne << std::endl;
  ...
  if (m_doGeant4)
      ...
      for (int iElectron = 0; iElectron<ne; iElectron++)
	{
	  ...
	  trackHeed->GetElectron(iElectron, xe, ye, ze, te, ee, dxe, dye, dze);
	  
	  //std::cout << "[UserG4FastSimulationModel] Initial Position is: x = " << xe << " cm\t y = " << ye << " cm\t z = " << ze << " cm" << std::endl;
  ...
  for(unsigned int iElectrons=0;iElectrons<electrons->size();iElectrons++)
    {
  ...
  if(m_doIons)
    {
      G4VUserTrackInformation* garfieldLabel = new G4VUserTrackInformation("GarfieldHit");
      for(unsigned int iIons=0;iIons<(*m_avalanche->GetIons()).size();iIons++)
  	{ 
	  ...
	  G4ThreeVector ionPosition(ion.GetX()*cm, ion.GetY()*cm, ion.GetZ()*cm);
	  G4ThreeVector electronPosition(electron.GetX()*cm, electron.GetY()*cm, electron.GetZ()*cm);
	  ...
	  navigator->SetWorldVolume(G4TransportationManager::GetTransportationManager()->
				    GetNavigatorForTracking()->GetWorldVolume());
	  navigator->LocateGlobalPointAndUpdateTouchableHandle(localPosition, 
							       localDirection, 
							       touchableHandle, 
							       false);
	  ...
	  if(!m_gotWFano)
	    {
	      fakePostStepPoint->SetSafety(trackHeed->GetW());
	      fakePostStepPoint->SetWeight(trackHeed->GetFanoFactor());
	      m_gotWFano=true;
	    }
	  ...
	  //std::cout << "[UserG4FastSimulationModel] Track label is: " << fakeStep->GetTrack()->GetUserInformation()->GetType() << std::endl;
	  //fakeStep->SetTotalEnergyDeposit(9*MeV); <-- Set energy deposit here if needed
	  
	  //std::cout << "[UserG4FastSimulationModel] LocalPosition: x = " << ionPosition.x()/cm << "cm, y = " << ionPosition.y()/cm << "cm, z = " << ionPosition.z()/cm << "cm, t = " << ion.GetTime()  << " ns, charge scale = " << ion.GetCharge() << std::endl;
	  //std::cout << "[UserG4FastSimulationModel] Step: x = " << fakeStep->GetPreStepPoint()->GetPosition().x()/cm << "cm, y = " << fakeStep->GetPreStepPoint()->GetPosition().y()/cm << "cm, z = " << fakeStep->GetPreStepPoint()->GetPosition().z()/cm << "cm, t = " << fakeStep->GetPreStepPoint()->GetGlobalTime()/ns << " ns, charge scale = " << ion.GetCharge() << std::endl; 
	  
	  G4VPhysicalVolume* currentVolume = fakeStep->GetPreStepPoint()->GetPhysicalVolume();
	  G4VSensitiveDetector* sensitiveDet;
	  
	  if (currentVolume != 0){
	    std::cout << "[UserG4FastSimulationModel] Physical Volume found! " << currentVolume->GetName() << std::endl;
	    sensitiveDet = currentVolume->GetLogicalVolume()->GetSensitiveDetector();
	    std::cout << "[UserG4FastSimulationModel] Logical Volume found! " << currentVolume->GetLogicalVolume()->GetName() << std::endl;
	    
	    if (sensitiveDet != 0){	
	      //std::cout << "[UserG4FastSimulationModel] SD collection name is: " << sensitiveDet->GetCollectionName(0) << std::endl;
  ...
  if(m_doSignal)
    {
      std::cout <<"[UserG4FastSimulationModel] Will drift Ions now " <<std::endl;
      m_drift->DriftIons(m_avalanche->GetIons());
    
      for(int ibin=0;ibin<m_signalnBins;ibin++)
	{
	  signalNt[ibin]+=m_sensors[0]->GetElectronSignal("readout",ibin);
	  signalNt[ibin]+=m_sensorIons->GetIonSignal("readout",ibin);
	}
      m_sensors[0]->ClearSignal();
      m_sensorIons->ClearSignal();
    }

  if(m_doElectronics)
    {m_pulseProcess->ProcessPulse(signalNt,signaltNt,pulseNt,pulsetNt);}
 
  for(unsigned int iElecs=0;iElecs<(*m_avalanche->GetElectrons()).size();iElecs++)
    {
      finElecRadiusNt.push_back((*m_avalanche->GetElectrons())[iElecs].GetRadius());
      finElecPhiNt.push_back((*m_avalanche->GetElectrons())[iElecs].GetPhi());
      finElecCosThetaNt.push_back((*m_avalanche->GetElectrons())[iElecs].GetCosTheta());
  ...
  if(m_doIons){
	...
	finIonCosThetaNt.push_back((*m_avalanche->GetIons())[iIons].GetCosTheta());
  ...
  //std::cout << "[UserG4FastSimulationModel] Number of electrons = " << electrons->size() << std::endl;

  timer.Stop();
  cpuTimeNt = timer.CpuTime();
  realTimeNt = timer.RealTime();
  std::cout << "[UserG4FastSimulationModel] Cpu time " << cpuTimeNt << std::endl; 
  std::cout << "[UserG4FastSimulationModel] Wall time " << realTimeNt << std::endl;
...
{
  std::cout << "[UserG4FastSimulationModel] Now initializing Garfield++" << std::endl;
  std::cout << "[UserG4FastSimulationModel] Initialisation Parameters are: " << std::endl;
  params->Print();
  ...
  double wValue     = params->GetWValue()/eV;
  double fanoFactor = params->GetFanoFactor();
  int debug       = params->GetDebug();
  bool doSafetyCounter = params->GetSafetyCounter();
  double pressure = params->GetPressure();
  
  signalNt.resize(m_signalnBins, 0);
  signaltNt.resize(m_signalnBins, 0);
  

  GasMixture* gas = new GasMixture();
  if(ionMobDir!="BasicMobility")
    gas->SetIonMobilityDir(ionMobDir);
  else
    gas->SetIonMobilityDir("");
  if(pressure)
    gas->SetGas(gasType, wValue, fanoFactor, pressure);
  else
    gas->SetGas(gasType, wValue, fanoFactor);
  FieldMap* fieldMap = new FieldMap();
  fieldMap->SetGas(gas->GetGas());
  fieldMap->SetSensorDir(sensorDir);
  fieldMap->Initialise();

  this->FindAmplificationArea(fieldMap->GetFieldMap());
  ...
  if(doSingleElec)
    m_avalanche->EnableSingleElectron();
  else
    m_avalanche->DisableSingleElectron();
  if(doSingleIon)
    m_avalanche->EnableSingleIon();
  else
    m_avalanche->DisableSingleIon();
  if(doSafetyCounter)
    m_avalanche->EnableSafetyCounter();
  else
    m_avalanche->DisableSafetyCounter();
  if(initAvalType=="Micro")
    {
  ...
  m_drift->SetSensor(m_sensorIons);
  if(ionDriftType=="RKF")
    m_drift->SetDriftType(DriftType::RKF);
  else if (ionDriftType=="MC")
    m_drift->SetDriftType(DriftType::MCdrift);
  else
    m_drift->SetDriftType(DriftType::RKF); //use RKF by default
  m_drift->SetDebug(debug);
  ...
  if (generationType=="Geant4")
      ...
      if(generationType=="HEED")
	{
	  ...
	  if (doAr37Calib)
	    m_initialElectrons->EnableAr37calib();
	}
  ...
  m_pulseProcess->SetTimeConstant(140.);
  m_pulseProcess->SetSamplingTime(0.48);
  m_pulseProcess->SetfCtomV(-1.);  

  if(doFieldPlot)
    CalculateEFieldDiff(fieldMap->GetFieldMap());
}
...
void UserG4FastSimulationModel::FindAmplificationArea(Garfield::ComponentAnsys121* ansysMap)
{
  ...
  // looking at field above the sensor (along y), where it should be strongest
  {
    Garfield::Medium* gasDummy;
    int statusDummy;
    std::cout<< "CHECKS of Efield " <<std::endl;
    double ex=0.,ey=0.,ez=0.;
    std::cout << "Sensor radius = " << sensor_radius << std::endl;
    double r= 2*sensor_radius;
    //double = 0.3;
    double phi = 1*acos(-1)/4;
    ansysMap->ElectricField(r*cos(phi),-5*sensor_radius,r*sin(phi),ex,ey,ez,gasDummy,statusDummy);
    ...
    std::cout<<" electric field at check point"<< " (Ex,Ey,Ez)= ("
	     <<ex << " "<<ey << " "<< ez<< "), Magnitude =  "<<sqrt(ex*ex+ey*ey+ez*ez)<<std::endl;
  }
  ...
  do 
    {
      ...
      ansysMap->ElectricField(0.0,radiusBisect,0.0,ex,ey,ez,gasDummy,statusDummy);
      std::cout<<" electric field at "<< radiusBisect/sensor_radius << " sensor radii or "<< radiusBisect<< " cm above sensor (Ex,Ey,Ez)= ("
	       <<ex << " "<<ey << " "<< ez<< "), Magnitude =  "<<sqrt(ex*ex+ey*ey+ez*ez)<<std::endl;
...
void UserG4FastSimulationModel::CreateSensors(Garfield::ComponentAnsys121* ansysMap)
{
  m_sensors.clear();
  delete m_sensorIons;
  //Create sensors for different strength analysis    
  // barArea is the area near sensor/rod where microcopic tracking of electrons is taking place
  // rest areas are filling the remaining of the sphere
  ...
  // intentionally made centralArea a bit larger, to avoid electrons going backwards too much
  // incomging electron will be relatively quite a bit in the final area when microscopic trcking begins.
  centralArea->SetArea(-aBar*1.05,-aCube,-aBar*1.05,aBar*1.05,aBar*1.05,aBar*1.05);
  ...
  //idealAreaPosY->SetArea(-aBar,aBar,-aCube,aBar,aCube,aCube);
  ...
  //idealAreaPosZ->SetArea(-aBar,-aCube,aBar,aBar,aBar,aCube);
  idealAreaPosZ->SetArea(-aCube,-aCube,aBar,aBar,aBar,aCube);
 
  Garfield::Sensor* idealAreaNegZ = new Garfield::Sensor();
  idealAreaNegZ->AddComponent(ansysMap);
  //idealAreaNegZ->SetArea(-aBar,-aCube,-aCube,aBar,aBar,-aBar);
  idealAreaNegZ->SetArea(-aCube,-aCube,-aCube,aBar,aBar,-aBar);
...
void UserG4FastSimulationModel::SetDetectorParameters(std::string sensorName)
{
  std::cout << "[UserG4FastSimulationModel] Setting Sensor Variables to match: " << sensorName << std::endl;
  if(sensorName=="BasicSensor")          //Need to check .mac file to confirm these
    {
      aCube = 15.;
      aBar = 0.3;
      sensor_radius = 0.1;
  ...
  else if(sensorName=="UmbrellaSensor") //(default sensor) Need to check .mac file to confirm these
    {
      aCube = 30.;
      aBar = 0.3;
      sensor_radius = 0.15;
  ...
  else if(sensorName=="UmbrellaSensor_1750V")
    {
      aCube = 30.;
      aBar = 0.3;
      sensor_radius = 0.15;
  ...
  else if(sensorName=="UmbrellaSensor_1800V")
    {
      aCube = 30.;
      aBar = 0.3;
      sensor_radius = 0.15;
  ...
  else if(sensorName=="UmbrellaSensor_1850V")
    {
      aCube = 30.;
      aBar = 0.3;
      sensor_radius = 0.15;
  ...
  else if(sensorName=="UmbrellaSensor_1900V")
    {
      aCube = 30.;
      aBar = 0.3;
      sensor_radius = 0.15;
    ...
    {
      std::cout << "[UserG4FastSimulationModel] ERROR: Unrecognised sensor name!" << std::endl;
    }
}

void UserG4FastSimulationModel::CalculateEFieldDiff(Garfield::ComponentAnsys121* ansysMap)
{
  ...
  double geometricConst = anode_voltage/(detector_radius-sensor_radius)*(detector_radius*sensor_radius);

  // Set up canvas and histogram
  TCanvas* c1 = new TCanvas("c1", "Relative difference between Ansys and Ideal E-Fields", 600, 600); 
  TH2F *h2d =  new TH2F("h2d", "Relative difference between Ansys and Ideal E-Fields; Radius (cm); cosTheta", nPoints, sensor_radius, detector_radius, nPoints, -1, 1);
  ...
  for(unsigned int iPoint = 0; iPoint<nPoints; iPoint++)
      ...
      for(unsigned int jPoint = 0; jPoint<nPoints; jPoint++)
	{
	  ...
	  ansysMap->ElectricField(radius*sin(acos(cosTheta)),radius*cosTheta,0.0,ex,ey,ez,gasDummy,statusDummy);
...
void UserG4FastSimulationModel::BookTree(int trackID)
{
  ...
  m_tree->Branch("IntegratedTownsend", &integrateTownsendNt, "IntegratedTownsend/D");
...
{  
  ...
  delete signalHistNt;
  signalHistNt =new TH1D("signalHistNt",":t[ns]:",m_signalnBins, m_signalt0, m_signalt0+m_signalBinSize*m_signalnBins);
  delete signalHistAvalNt;
  signalHistAvalNt =new TH1D("signalHistAvalNt",":t[ns]:",m_signalnBins, m_signalt0, m_signalt0+m_signalBinSize*m_signalnBins);
}
