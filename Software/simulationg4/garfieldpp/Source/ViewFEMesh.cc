...
void ViewFEMesh::SetComponent(ComponentFieldMap* cmp) {
  ...
  //m_component = cmp;
  if(!m_component)
    m_component=new std::vector<ComponentFieldMap*>;
  m_component->push_back(cmp);
...
bool ViewFEMesh::Plot() {
  ...
  if (!m_component->at(0)->GetVoltageRange(pmin, pmax)) {
  ...
  // Plot the elements
  ComponentCST* componentCST = dynamic_cast<ComponentCST*>(m_component->at(0));
  if (componentCST) {
...
bool ViewFEMesh::GetPlotLimits() {
  ...
  if (!m_userBox) {
    ...
    if (!m_component) {
      ...
      if (!m_component->at(0)->GetBoundingBox(m_xMinBox, m_yMinBox, m_zMinBox,
...
void ViewFEMesh::DrawElements() {
  ...
  double mapxmax = m_component->at(0)->m_mapmax[0];
  double mapxmin = m_component->at(0)->m_mapmin[0];
  double mapymax = m_component->at(0)->m_mapmax[1];
  double mapymin = m_component->at(0)->m_mapmin[1];
  double mapzmax = m_component->at(0)->m_mapmax[2];
  double mapzmin = m_component->at(0)->m_mapmin[2];
  ...
  const bool perX =
      m_component->at(0)->m_periodic[0] || m_component->at(0)->m_mirrorPeriodic[0];
  const bool perY =
      m_component->at(0)->m_periodic[1] || m_component->at(0)->m_mirrorPeriodic[1];
  const bool perZ =
      m_component->at(0)->m_periodic[2] || m_component->at(0)->m_mirrorPeriodic[2];
  ...
  for (const auto& element : m_component->at(0)->m_elements) {
    const auto mat = element.matmap;
    // Do not plot the drift medium.
    if (m_component->at(0)->m_materials[mat].driftmedium && !m_plotMeshBorders) {
      continue;
    }
    ...
    const auto& n0 = m_component->at(0)->m_nodes[element.emap[0]];
    const auto& n1 = m_component->at(0)->m_nodes[element.emap[1]];
    const auto& n2 = m_component->at(0)->m_nodes[element.emap[2]];
    const auto& n3 = m_component->at(0)->m_nodes[element.emap[3]];
    // Loop over the periodicities in x.
    for (int nx = nMinX; nx <= nMaxX; nx++) {
      // Determine the x-coordinates of the tetrahedral vertices.
      if (m_component->at(0)->m_mirrorPeriodic[0] && nx != 2 * (nx / 2)) {
      ...
      for (int ny = nMinY; ny <= nMaxY; ny++) {
        ...
        if (m_component->at(0)->m_mirrorPeriodic[1] && ny != 2 * (ny / 2)) {
        ...
        for (int nz = nMinZ; nz <= nMaxZ; nz++) {
          ...
          if (m_component->at(0)->m_mirrorPeriodic[2] && nz != 2 * (nz / 2)) {
