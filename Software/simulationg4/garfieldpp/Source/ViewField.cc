...
void ViewField::SetComponent(Component* c) {
  ...
  if(!m_component)
    m_component=new std::vector<Component*>;
...
void ViewField::Draw2d(const std::string& option, const bool contour, 
  ...
  if (wfield) {
    ...
    if (par == Parameter::Potential) {
      if (m_useAutoRange) {
        if (m_component) {
          if (m_samplePotential || !m_component->at(0)->GetVoltageRange(zmin, zmax)) {
...
void ViewField::DrawProfile(const double x0, const double y0, const double z0,
  ...
  if (wfield) {
    ...
    if (par == Parameter::Potential) {
      if (m_useAutoRange) {
        if (m_component) {
          if (m_samplePotential || !m_component->at(0)->GetVoltageRange(fmin, fmax)) {
            SampleRange(&f1, fmin, fmax);
          }
        } else if (m_sensor) {
...
bool ViewField::SetPlotLimits() {
  ...
  if (m_sensor) {
    ...
    ok = PlotLimits(m_component->at(0), xmin, ymin, xmax, ymax);
  } 
  if (ok) {
    m_xMinPlot = xmin;
    m_xMaxPlot = xmax;
...
double ViewField::Field(const double x, const double y, const double z,
  ...
  if (!m_sensor) {
    m_component->at(0)->ElectricField(x, y, z, ex, ey, ez, volt, medium, status);
  } else {
...
double ViewField::Wfield(const double x, const double y, const double z,
  ...
  if (par == Parameter::Potential) {
    if (m_sensor) {
      ...
      return m_component->at(0)->WeightingPotential(x, y, z, electrode);
    }
  }

  double ex = 0., ey = 0., ez = 0.;
  if (!m_sensor) {
    m_component->at(0)->WeightingField(x, y, z, ex, ey, ez, electrode);
  } else {
...
void ViewField::PlotFieldLines(const std::vector<double>& x0,
  ...
  if (m_sensor) {
    ...
    if (!m_component->at(0)->GetBoundingBox(xmin, ymin, zmin, xmax, ymax, zmax)) {
      if (m_userBox) {
        sensor.SetArea(m_xMinBox, m_yMinBox, m_zMinBox, 
                       m_xMaxBox, m_yMaxBox, m_zMaxBox);
      }
    }
    sensor.AddComponent(m_component->at(0));
...
bool ViewField::EqualFluxIntervals(
  ...
  double q = 0.;
  if (m_component) {
    q = m_component->at(0)->IntegrateFluxLine(x0, y0, z0, x1, y1, z1, 
					      xp, yp, zp, 20 * nV, 0);
  } else {
  ...
  for (size_t i = 0; i < nSteps; ++i) {
    ...
    if (m_component) {
      q = m_component->at(0)->IntegrateFluxLine(x, y, z, x + dx, y + dy, z + dz, 
                                         xp, yp, zp, nV, isign);
    } else {
