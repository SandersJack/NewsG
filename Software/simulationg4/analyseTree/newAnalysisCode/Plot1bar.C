...
int main()
{
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++)
      ...
      for (int i=nPulse.size()-1; i>0; i--)
	{
	 ...
	 h_nPulseIntegral4->Fill(PulseIntegral);
      ...
      //Region 3
	 else if (Amplitude>2600 && RiseTime<10)
	{
	  R3Events += 1;
	  for (int elecNumber = 0; elecNumber<Radius.size(); elecNumber++)
	    {
      ...
      //Region 2
      else if (RiseTime>-0.07*Amplitude+210)
	{
	  R2Events += 1;
	  for (int elecNumber = 0; elecNumber<Radius.size(); elecNumber++)
	    {
  ...
  TCanvas*c26=new TCanvas("c26","Initial Electron Position 5",200,600);
