...
:m_detectorConstruction(det)
{
  ...
  m_cathodeMaterialCmd = new G4UIcmdWithAString("/det/cathodeMaterial", this);
  m_cathodeMaterialCmd->SetGuidance("Set the material for the cathode (outer sphere)");
  m_cathodeMaterialCmd->SetParameterName("Material", false, false);
  m_cathodeMaterialCmd->AvailableForStates(G4State_PreInit, G4State_Idle);

  m_defaultMaterialCmd = new G4UIcmdWithAString("/det/defaultMaterial", this);
  m_defaultMaterialCmd->SetGuidance("Set the default material (used outside sphere)");
  m_defaultMaterialCmd->SetParameterName("Material", false, false);
  m_defaultMaterialCmd->AvailableForStates(G4State_PreInit, G4State_Idle);

  m_idealFieldCmd = new G4UIcmdWithABool("/det/garfield/idealField", this);
  ...
  m_useGarfieldCmd = new G4UIcmdWithABool("/det/garfield/useGarfield", this);
  m_useGarfieldCmd->SetGuidance("Use Garfield++ model (1) or switch it off, e.g for visualisation (0).");
  m_useGarfieldCmd->SetParameterName("State", false, false);
  m_useGarfieldCmd->AvailableForStates(G4State_PreInit, G4State_Idle);

  m_srimFileCmd = new G4UIcmdWithAString("/det/garfield/setSrimFile", this);
  m_srimFileCmd->SetGuidance("Set SRIM file to be used if one is needed.");
  m_srimFileCmd->SetGuidance("(Ensure the file you need has been made!)");
  m_srimFileCmd->SetParameterName("File", false, false);
  m_srimFileCmd->AvailableForStates(G4State_PreInit, G4State_Idle);

  // m_detectorRadius = new G4UIcmdWithADoubleAndUnit("/det/setDetectorRadius", this);
  // m_detectorRadius->SetGuidance("Set the radius of the detector [cm] (cathode inner radius)");
  // m_detectorRadius->SetDefaultUnit("cm");
  // m_detectorRadius->SetParameterName("detectorRadius", false, false);
  // m_detectorRadius->AvailableForStates(G4State_PreInit, G4State_Idle);


  // m_anodeRadius = new G4UIcmdWithADoubleAndUnit("/det/setAnodeRadius", this);
  // m_anodeRadius->SetGuidance("Set the radius of the anode [cm] (anode outer radius)");
  // m_anodeRadius->SetDefaultUnit("cm");
  // m_anodeRadius->SetParameterName("anodeRadius", false, false);
  // m_anodeRadius->AvailableForStates(G4State_PreInit, G4State_Idle);
...
DetectorConstructionMessenger::~DetectorConstructionMessenger()
{
  ...
  delete m_useGarfieldCmd;
  delete m_pressureCmd;
  delete m_garfieldDir;
  delete m_detDir;
  delete m_srimFileCmd;
}
...
void DetectorConstructionMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  if(command == m_idealFieldCmd)
    {m_detectorConstruction->SetIdealField(m_idealFieldCmd->GetNewBoolValue(newValue));}
  if(command == m_useGarfieldCmd)
    {m_detectorConstruction->SetUseGarfield(m_useGarfieldCmd->GetNewBoolValue(newValue));}
  if(command == m_pressureCmd)
    {m_detectorConstruction->SetPressure(m_pressureCmd->GetNewDoubleValue(newValue));}
  if (command == m_cathodeMaterialCmd) {
    m_detectorConstruction->SetCathodeMaterial(newValue);
  }
  if (command == m_defaultMaterialCmd) {
    m_detectorConstruction->SetDefaultMaterial(newValue);
  }
  if (command == m_srimFileCmd) {
    m_detectorConstruction->SetSrimFile(newValue);
  }
