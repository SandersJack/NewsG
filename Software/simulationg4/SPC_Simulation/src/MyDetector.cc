...
G4bool MyDetector::ProcessHits(G4Step* aStep, G4TouchableHistory* /*ROhist*/)
{
  ...
  G4int status = aStep->GetPreStepPoint()->GetStepStatus();
  ...
  aHit->SetElecCharge(elecCharge);
  G4int elecStatus = aStep->GetPostStepPoint()->GetStepStatus();
  aHit->SetElecStatus(elecStatus);
  G4double avgGain = aStep->GetPreStepPoint()->GetSafety();
  aHit->SetGain(avgGain);
