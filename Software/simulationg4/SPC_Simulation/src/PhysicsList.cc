  ...
  : G4VModularPhysicsList()
{
  ...
  hadPhysicsList->push_back(new G4RadioactiveDecayPhysics());
  //removed for nBLM
...
void PhysicsList::AddParameterisation() {
  ...
  G4FastSimulationManagerProcess* FSprocess =
  ...
  while ((*it)()) {
    ...
    if (p->GetParticleName() == "e-") {
      ...
      config->SetExtraEmModel("e-", "eIoni", pai, "gas_region", 0. * eV,
                              1. * TeV, pai);
      std::cout << "[PhysicsList] Added PAI model for e-" << std::endl;
    }
    else if (m_use_srim_protons && p->GetParticleName() == "proton") {
      manager->AddDiscreteProcess(FSprocess);
      G4PAIModel* pai = new G4PAIModel(p, "G4PAIModelProton");
      config->SetExtraEmModel("proton", "hIoni", pai, "gas_region", 0. * eV,
                              1. * TeV,
                              pai);  // Check if the names need to be different
    ...
    // config->SetExtraEmModel("GenericIon", "ionIoni", pai,
    // "DefaultRegionForTheWorld", 0.*eV, 1.*TeV, pai); //Causes segFaults
    // std::cout << "[PhysicsList] Added PAI model for GenericIon" <<
    // std::endl;

    else if (m_use_srim_alphas && p->GetParticleName() == "alpha") {
      manager->AddDiscreteProcess(FSprocess);
      G4PAIModel* pai = new G4PAIModel(p, "G4PAIModelAlpha");
      config->SetExtraEmModel("alpha", "ionIoni", pai, "gas_region", 0. * eV,
                              1. * TeV, pai);
