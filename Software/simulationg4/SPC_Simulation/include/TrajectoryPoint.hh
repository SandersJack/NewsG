...
public: 
  ...
  TrajectoryPoint();
  TrajectoryPoint(G4ThreeVector pos,const G4Material* mat);
  explicit TrajectoryPoint(const TrajectoryPoint &right);
  virtual ~TrajectoryPoint();
 
  // Operators
  inline void *operator new(size_t);
  inline void operator delete(void *aTrajectoryPoint);
  inline int operator==(const TrajectoryPoint& right) const {return (this==&right);}
 
  // Get/Set functions
  inline virtual const G4ThreeVector GetPosition() const override {return fPosition;}
...
extern G4ThreadLocal G4Allocator<TrajectoryPoint> * faTrajPointAllocator;
 
inline void* TrajectoryPoint::operator new(size_t)
{
  if(!faTrajPointAllocator)
    faTrajPointAllocator = new G4Allocator<TrajectoryPoint>;
  return (void *) faTrajPointAllocator->MallocSingle();
}
 
inline void TrajectoryPoint::operator delete(void *aTrajectoryPoint)
{
  faTrajPointAllocator->FreeSingle((TrajectoryPoint *) aTrajectoryPoint);
}
