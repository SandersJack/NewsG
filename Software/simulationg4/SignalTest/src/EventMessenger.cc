...
void EventMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  if(command == m_setSignalBinSizeCmd)
    {m_eventAction->SetSignalBinSize(m_setSignalBinSizeCmd->GetNewDoubleValue(newValue));}
  if(command == m_setnSignalBinsCmd)
    {m_eventAction->SetnSignalBins(m_setnSignalBinsCmd->GetNewIntValue(newValue));}
